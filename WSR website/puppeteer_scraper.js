...
function parseDuration(durationText) {
    // Convert YouTube duration text (e.g., "3:45") to seconds
    if (!durationText || durationText === 'Brevemente') return 0;
...
function calculateBPM(title) {
    ...
    // Default BPMs for different genres (estimated)
    const titleLower = title.toLowerCase();
    if (titleLower.includes('house') || titleLower.includes('progressive')) return 128;
    if (titleLower.includes('techno')) return 130;
    ...
    if (titleLower.includes('ambient') || titleLower.includes('chill')) return 90;
    
    return 128; // Default BPM
}
